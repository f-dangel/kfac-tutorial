...
\item \textbf{[Intuition] Show how Kronecker products naturally emerge in curvature matrices (the `KF' in KFAC).}
  Doing so provides a motivation for using Kronecker-factorization as `natural' structure to approximate it, which is what KFAC does (\cref{sec:kfac-overview}).
