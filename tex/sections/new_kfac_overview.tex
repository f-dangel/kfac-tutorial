...
The Kronecker strucutre in KFAC arises naturally in the expression for $\mC(\vtheta^{(i)})$ due to structure of the layer Jacobians, providing a strong motivation for its use. 
...
We have already seen this in previous sections, such as in the output-weight Jacobian of a linear layer, where the Kronecker structure arises explicitly (\Cref{jacobians_linear_layer}), with one of the Kronecker factors given by the layer input $\vx^{(i-1)}_n$. 
