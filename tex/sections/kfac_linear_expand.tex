...
In the broader KFAC nomenclature (\Cref{sec:preface}), we focus on the \emph{KFAC-expand approximation}, which corresponds to the seminal method proposed by~\citet{martens2015optimizing}.
...
We now derive the KFAC-expand approximation for a linear layer with weights $\mW$, input vector $\vx$, and output vector $\vz = \mW\vx$ (\Cref{setup:linear_layer}).
...
Applying \Cref{def:kfac_exp_approx}, we obtain the curvature matrix's KFAC-expand approximation:
\begin{align*}
  \mC&(\cvec \mW) \\
  \approx&
...
Had we done the same derivation using $\rvec$-flattening scheme where the output-parameter Jacobian takes the form (\Cref{ex:linear_layer_jacobians})
...
\begin{test}[KFAC-expand for linear layers in an MLP with one data point, \Cref{kfac_tests/expand_mlp_batch_size_1}]\label{test:kfac_expand_linear_no_weight_sharing_batch_size_1}
...
\begin{test}[KFAC-expand for regression with a deep linear net (no nonlinear layers), \Cref{kfac_tests/expand_deep_linear_regression}]
...
Although weight sharing is not of our interest in this chapter, we have already included such a slightly more general test case in \Cref{kfac_tests/expand_deep_linear_regression}.
