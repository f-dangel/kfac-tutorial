...
The inputs are processed by a neural net $f\colon \Theta \times \gX \to \gF$ and their predictions are scored with a criterion function $c\colon \gF \times \gY \to \sR$, like square loss (\Cref{ex:square_loss}) or softmax cross-entropy loss (\Cref{ex:cross_entropy_loss}).
