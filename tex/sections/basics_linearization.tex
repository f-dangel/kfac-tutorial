...
We know that convexity is preserved under composition with linear function, so if  $g$ is convex and $h$ is linear, then $f$ will be convex.
But what if $g$ is convex, but $h$ is not linear?
Well, then we can replace $h$ with an approximation $\bar{h}$ that is linear and approximates the original function.
...
If we use the chain rule twice, we obtain $f'' = h' \times g'' \circ h \times  h' + h'' \times g' \circ h$. Using our Jacobian and Hessian notation, this translates into
\begin{align*}
  &\hess_x f(x)
  \\
  &=\hess_x (g \circ h)(x)
