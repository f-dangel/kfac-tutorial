...
As previously described in \Cref{sec:partial_linearization}, the GGN matrix arises from partial linearization of a composite function.
...
\Cref{ex:mseloss_hessian_factorization,ex:crossentropyloss_hessian_factorization} present the loss Hessian factorizations of square and softmax cross-entropy loss.
One important insight for these two loss functions is that $\mS^{\vec}$ does not depend on the labels.
We will use this property in \Cref{subsec:connection-ggn-fisher} to connect the GGN with the Fisher for regression and classification.
